  ...
  Future<void> _exportPDF(TransactionProvider provider) async {
    ...
    try {
      ...
      final transactions = (await provider.streamTransactions().first)
          .where((t) => t.date.isAfter(_startDate.subtract(const Duration(days: 1))) &&
          ...
          build: (pw.Context context) {
            return pw.Column(
              children: [
                ...
                pw.Row(
                  children: [
                    pw.Text('User: ${FirebaseAuth.instance.currentUser?.email ?? "N/A"}'),
                  ],
                ),
                pw.SizedBox(height: 5),
                pw.Row(
                  children: [
                    pw.Text('Date Range: ${_startDate.toString().split(' ')[0]} to ${_endDate.toString().split(' ')[0]}'),
                  ],
                ),
                  ...
                  children: [
                    ...
                    ...transactions.map((t) => pw.TableRow(
                      children: [
                        ...
                        pw.Text(t.type),
                        pw.Text('Rp ${t.amount.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'),
                        pw.Text(t.note),
                      ],
                    )).toList(),
                  ],
                ...
                pw.Row(
                  ...
                  children: [
                    pw.Text('Total Income: Rp ${transactions.where((t) => t.type == 'income').fold(0.0, (sum, t) => sum + t.amount).round().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'),
                    pw.Text('Total Expense: Rp ${transactions.where((t) => t.type == 'expense').fold(0.0, (sum, t) => sum + t.amount).round().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'),
                  ],
                ),
              ],
            );
      ...
      final file = File('${directory.path}/transactions_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
